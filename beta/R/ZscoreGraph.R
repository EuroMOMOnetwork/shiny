...
#' @return graph
#' @export
ZscoreGraph <- function(dt, c, a, g) {
  library(data.table)
  library(ggplot2)

  dt <-  setDT(dt)[(country == c) & (aggregation == a) & (group == g), .(zscore = (nbc - pnb)/sqrt(Vexcess)), keyby = ISOweek]

  dt$wk = as.numeric(as.factor(dt$ISOweek))
  graph <- ggplot(dt, aes(x = wk)) +
    geom_line(aes(y = zscore), linetype="solid", color="darkgreen") +
    ...
    scale_x_continuous(name = "ISOWeek",
                       labels = dt[seq(min(dt$wk), max(dt$wk), by = 13),]$ISOweek,
                       breaks = seq(min(dt$wk), max(dt$wk), by = 13)) +
